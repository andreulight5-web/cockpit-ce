...
import { QUIZ_5 } from '../../data/quiz-5'
...
export default function QuizFive() {
  ...
  const question = QUIZ_5.questions[qIdx]
  const total = QUIZ_5.questions.length
  ...
  const handleNext = () => {
    ...
    if (qIdx + 1 < total) {
      ...
      if (saveData) {
        ...
        const newQuizDone = quizDone.some((x) => String(x) === '5') ? quizDone : [...quizDone, 5]
        saveData({
    ...
    <div style={s.page}>
      ...
      <header style={s.header}>
        ...
        <div style={s.headerCenter}>
          <div style={s.headerTitle}>{QUIZ_5.title}</div>
          {step === 'question' && (
...
function Intro({ onStart }) {
  return (
    <div style={s.card}>
      <p style={s.videoDesc}>{QUIZ_5.videoDescription}</p>
      ...
      <h1 style={s.introTitle}>{QUIZ_5.title}</h1>
      <p style={s.introDesc}>
