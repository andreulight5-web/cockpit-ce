...
export default function QuizOne() {
  ...
  const question = QUIZ_1.questions[qIdx]
  const total = QUIZ_1.questions.length
  ...
  const handleNext = () => {
    ...
    if (qIdx + 1 < total) {
      ...
      if (saveData) {
        ...
        const newQuizDone = quizDone.some((x) => String(x) === '1') ? quizDone : [...quizDone, 1]
        saveData({
    ...
    <div style={s.page}>
      ...
      <header style={s.header}>
        ...
        <div style={s.headerCenter}>
          <div style={s.headerTitle}>{QUIZ_1.title}</div>
          {step === 'question' && (
      ...
      <main style={s.body}>
        ...
        {step === 'recap' && (
          <Recap defi={answers[5]?.value} onNext={() => navigate('/quiz/2')} onSommaire={() => navigate('/quiz')} />
        )}
      </main>
    </div>
  )
...
function Intro({ onStart }) {
  return (
    <div style={s.card}>
      <p style={s.videoDesc}>{QUIZ_1.videoDescription}</p>
      ...
      <h1 style={s.introTitle}>{QUIZ_1.title}</h1>
      <p style={s.introDesc}>
