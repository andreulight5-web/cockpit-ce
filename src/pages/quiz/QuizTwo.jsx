...
import { QUIZ_2 } from '../../data/quiz-2'
...
export default function QuizTwo() {
  ...
  const question = QUIZ_2.questions[qIdx]
  const total = QUIZ_2.questions.length
  ...
  const handleNext = () => {
    ...
    if (qIdx + 1 < total) {
      ...
      if (saveData) {
        ...
        const newQuizDone = quizDone.some((x) => String(x) === '2') ? quizDone : [...quizDone, 2]
        saveData({
    ...
    <div style={s.page}>
      ...
      <header style={s.header}>
        ...
        <div style={s.headerCenter}>
          <div style={s.headerTitle}>{QUIZ_2.title}</div>
          {step === 'question' && (
...
function Intro({ onStart }) {
  return (
    <div style={s.card}>
      <p style={s.videoDesc}>{QUIZ_2.videoDescription}</p>
      ...
      <h1 style={s.introTitle}>{QUIZ_2.title}</h1>
      <p style={s.introDesc}>
