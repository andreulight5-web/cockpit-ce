...
import { QUIZ_3 } from '../../data/quiz-3'
...
export default function QuizThree() {
  ...
  const question = QUIZ_3.questions[qIdx]
  const total = QUIZ_3.questions.length
  ...
  const handleNext = () => {
    ...
    if (qIdx + 1 < total) {
      ...
      if (saveData) {
        ...
        const newQuizDone = quizDone.some((x) => String(x) === '3') ? quizDone : [...quizDone, 3]
        saveData({
    ...
    <div style={s.page}>
      ...
      <header style={s.header}>
        ...
        <div style={s.headerCenter}>
          <div style={s.headerTitle}>{QUIZ_3.title}</div>
          {step === 'question' && (
      ...
      <main style={s.body}>
        ...
        {step === 'recap' && (
          <Recap defi={answers[5]?.value} onNext={() => navigate('/quiz/4')} onSommaire={() => navigate('/quiz')} />
        )}
      </main>
    </div>
  )
...
function Intro({ onStart }) {
  return (
    <div style={s.card}>
      <p style={s.videoDesc}>{QUIZ_3.videoDescription}</p>
      ...
      <h1 style={s.introTitle}>{QUIZ_3.title}</h1>
      <p style={s.introDesc}>
