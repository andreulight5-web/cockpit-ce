...
function LeconInner({ id }) {
  ...
  if (!lecon) return <div style={{ background: '#FAFAF5', color: '#1C1B2E', minHeight: '100dvh', padding: 40 }}>Lecon introuvable</div>
  ...
  return (
    <div style={{ background: '#FAFAF5', minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      <div style={S.header}>
        ...
        <div style={S.headerRight}>
          ...
          {!showAfter && <div style={S.headerDots}>{lecon.cartes.map((_, i) => <span key={i} style={{ width: 5, height: 5, borderRadius: '50%', background: i <= cardIdx ? '#2A9490' : '#E5E5E5' }} />)}</div>}
        </div>
      </div>
      ...
      {showAfter && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 20px 80px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 28 }}>
            ...
            <button onClick={() => navigate(backPath)} style={{ ...S.btn, background: 'transparent', border: '1px solid rgba(28,27,46,0.12)', color: '#1C1B2E' }}>Retour au programme</button>
          </div>
        </div>
      )}
    </div>
...
function RenderCard({ carte: c, color, cortex, lecon, onShowScenario, onComplete, basePath }) {
  ...
  switch (c.type) {
    ...
    case 'fact2col': return (
      <div style={{ ...S.cardFull, background: '#FAFAF5', padding: '72px 16px 80px' }}>
    ...
    case 'contraste': return (
      <div style={{ ...S.cardFull, background: '#FAFAF5', padding: '72px 20px 80px', justifyContent: 'center' }}>
        ...
        <div style={{ ...S.contCard, borderColor: '#C0506A', background: 'rgba(212,83,126,0.10)' }}>
          ...
          <p style={{ fontFamily: 'Poppins,sans-serif', fontSize: 16, color: '#1C1B2E', fontStyle: 'italic', lineHeight: 1.5, margin: '8px 0 0' }}>{c.gauche.texte}</p>
        </div>
        <div style={{ ...S.contCard, borderColor: '#2A9490', background: 'rgba(42,148,144,0.10)', marginTop: 10 }}>
          ...
          <p style={{ fontFamily: 'Poppins,sans-serif', fontSize: 16, color: '#1C1B2E', fontStyle: 'italic', lineHeight: 1.5, margin: '8px 0 0' }}>{c.droite.texte}</p>
        </div>
        {c.difference && <p style={{ fontSize: 15, color: '#64748B', lineHeight: 1.5, marginTop: 16, textAlign: 'center' }}>{c.difference}</p>}
      </div>
    )
      ...
      return (
        <div style={{ ...S.cardFull, background: '#FAFAF5', padding: '72px 20px 80px' }}>
          ...
          <div style={{ marginTop: 'auto', paddingTop: 24, width: '100%' }}>
            ...
            {lecon.scenario && onShowScenario && <button onClick={onShowScenario} style={{ ...S.btn, background: 'transparent', border: '1px solid rgba(255,255,255,0.2)', color: '#64748B', width: '100%', borderRadius: 50, padding: '12px 28px', marginTop: 8 }}>Scenario du jour</button>}
          </div>
        </div>
      )
    }
...
function Scenario({ scenario, color, openAccordion, setOpenAccordion }) {
  const sections = [
    { key: 'situation', icon: '🎬', label: 'La situation', content: scenario.situation, style: { fontStyle: 'italic', color: '#1C1B2E' } },
        ...
        return (
          <button key={sec.key} onClick={() => setOpenAccordion(open ? null : sec.key)} style={S.accBtn}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontFamily: 'Poppins,sans-serif', fontSize: 14, fontWeight: 600, color: '#1C1B2E' }}>{sec.label}</span>
              <span style={{ marginLeft: 'auto', fontSize: 14, color: '#999', transform: open ? 'rotate(90deg)' : 'none', transition: 'transform 0.2s' }}>›</span>
            </div>
            {open && (
              <div style={{ marginTop: 10 }}>
                ...
                {sec.key === 'demain' && scenario.demain.map((d, i) => (
                  <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 8 }}>
                    ...
                    <p style={{ fontSize: 13, color: '#1C1B2E', lineHeight: 1.5, margin: 0 }}>{d}</p>
                  </div>
                ))}
              </div>
            )}
...
const S = {
  ...
  cardCount: { fontFamily: 'Inter,sans-serif', fontSize: 11, color: '#999' },
  ...
  thinSep: { height: 1, background: 'rgba(28,27,46,0.06)', margin: '2px 0' },
  ...
  parentBadge: { display: 'inline-block', marginTop: 6, background: 'rgba(28,27,46,0.08)', borderRadius: 99, padding: '3px 10px', fontSize: 9, color: '#64748B', letterSpacing: 0.5 },
  ...
  accBtn: { display: 'block', width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(28,27,46,0.06)', borderRadius: 14, padding: '14px 16px', marginBottom: 8, cursor: 'pointer', textAlign: 'left' },
}
